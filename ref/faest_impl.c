...
ATTR_PURE static inline unsigned int quicksilver_degree(const faest_paramset_t* params) {
  return faest_is_lynx(params) ? 2 : 3;
}

ATTR_PURE static inline unsigned int quicksilver_row_bits(const faest_paramset_t* params) {
  return params->l + quicksilver_degree(params) * params->lambda + UNIVERSAL_HASH_B_BITS;
}

ATTR_PURE static inline unsigned int ell_hat_bytes(const faest_paramset_t* params) {
...
ATTR_PURE static inline uint8_t* signature_a1_tilde(uint8_t* base_ptr,
  ...
  return base_ptr + (params->tau - 1) * ell_hat_bytes(params) + utilde_bytes + params->l / 8;
}
...
static void hash_challenge_1(uint8_t* chall_1, const uint8_t* mu, const uint8_t* hcom,
                             const uint8_t* c, const uint8_t* iv,
                             const faest_paramset_t* params) {
  const unsigned int lambda_bytes = params->lambda / 8;

  H2_context_t h2_ctx;
  H2_init(&h2_ctx, params->lambda);
  H2_update(&h2_ctx, mu, lambda_bytes * 2);
  H2_update(&h2_ctx, hcom, lambda_bytes * 2);
  H2_update(&h2_ctx, c, ell_hat_bytes(params) * (params->tau - 1));
...
static void hash_challenge_3(uint8_t* chall_3, const uint8_t* chall_2, const uint8_t* a0_tilde,
                             ...
                             const faest_paramset_t* params) {
  H2_context_t h2_ctx;
  hash_challenge_3_init(&h2_ctx, chall_2, a0_tilde, a1_tilde, a2_tilde, params);
  H2_update(&h2_ctx, ctr, sizeof(uint32_t));
  H2_3_final(&h2_ctx, chall_3, params->lambda / 8);
}
...
static inline void aes_prove(uint8_t* a0_tilde, uint8_t* a1_tilde, uint8_t* a2_tilde,
                             ...
                             const faest_paramset_t* params) {
  if (faest_is_lynx(params)) {
    if (params->lambda == 128) {
      lynx_128_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
      return;
    }
    if (params->lambda == 192) {
      lynx_192_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
      return;
    }
    if (params->lambda == 256) {
      lynx_256_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
      return;
    }
    if (params->lambda == 384) {
      lynx_384_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
      return;
    }
    if (params->lambda == 512) {
      lynx_512_prover(a0_tilde, a1_tilde, a2_tilde, w, u, V, owf_in, owf_out, chall_2, params);
      return;
    }
  }
...
static inline void aes_verify(uint8_t* a0_tilde, const uint8_t* d, uint8_t** Q,
  ...
  if (faest_is_lynx(params)) {
    if (params->lambda == 128) {
      lynx_128_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde,
                        params);
      return;
    }
    if (params->lambda == 192) {
      lynx_192_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde,
                        params);
      return;
    }
    if (params->lambda == 256) {
      lynx_256_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde,
                        params);
      return;
    }
    if (params->lambda == 384) {
      lynx_384_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde,
                        params);
      return;
    }
    if (params->lambda == 512) {
      lynx_512_verifier(a0_tilde, d, Q, owf_in, owf_out, chall_2, chall_3, a1_tilde, a2_tilde,
                        params);
      return;
    }
  }
...
void faest_sign(uint8_t* sig, const uint8_t* msg, size_t msg_len, const uint8_t* owf_key,
  ...
  hash_challenge_1(chall_1, mu, bavc.h, signature_c(sig, 0, params), iv, params);
  ...
  uint8_t a2_tilde_dummy[MAX_LAMBDA_BYTES] = {0};
  uint8_t* a2_tilde = faest_is_lynx(params) ? a2_tilde_dummy : signature_a2_tilde(sig, params);
  aes_prove(a0_tilde, signature_a1_tilde(sig, params), a2_tilde, witness, u + ell_bytes, V,
...
int faest_verify(const uint8_t* msg, size_t msglen, const uint8_t* sig, const uint8_t* owf_input,
  ...
  // ::10
  uint8_t chall_1[5 * MAX_LAMBDA_BYTES + 8];
  hash_challenge_1(chall_1, mu, hcom, dsignature_c(sig, 0, params), iv, params);
  ...
  uint8_t a0_tilde[MAX_LAMBDA_BYTES];
  uint8_t a2_tilde_dummy[MAX_LAMBDA_BYTES] = {0};
  const uint8_t* a2_tilde = faest_is_lynx(params) ? a2_tilde_dummy : dsignature_a2_tilde(sig, params);
  aes_verify(a0_tilde, d, q, chall_2, dsignature_chall_3(sig, params),
             dsignature_a1_tilde(sig, params), a2_tilde, owf_input, owf_output, params);
  free_pointer_array(&q);

  // Step: 20
  uint8_t chall_3[MAX_LAMBDA_BYTES];
  hash_challenge_3(chall_3, chall_2, a0_tilde, dsignature_a1_tilde(sig, params), a2_tilde,
