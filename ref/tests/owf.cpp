...
namespace {
  ...
  void test_owf_tv(const faest_paramid_t param_id, const std::array<uint8_t, KeySize>& owf_key,
    ...
    case FAEST_LYNX_128S:
      owf_lynx_128(owf_key.data(), owf_input.data(), owf_output.data());
      break;
    case FAEST_LYNX_192S:
      owf_lynx_192(owf_key.data(), owf_input.data(), owf_output.data());
      break;
    case FAEST_LYNX_256S:
      owf_lynx_256(owf_key.data(), owf_input.data(), owf_output.data());
      break;
    case FAEST_LYNX_384S:
      owf_lynx_384(owf_key.data(), owf_input.data(), owf_output.data());
      break;
    default:
      BOOST_TEST(false);
    }
...
BOOST_AUTO_TEST_CASE(tv_owf_lynx_128) {
  test_owf_tv(FAEST_LYNX_128S, owf_tvs::FAEST_LYNX_128S::key, owf_tvs::FAEST_LYNX_128S::input,
              owf_tvs::FAEST_LYNX_128S::output);
}

BOOST_AUTO_TEST_CASE(tv_owf_lynx_192) {
  test_owf_tv(FAEST_LYNX_192S, owf_tvs::FAEST_LYNX_192S::key, owf_tvs::FAEST_LYNX_192S::input,
              owf_tvs::FAEST_LYNX_192S::output);
}

BOOST_AUTO_TEST_CASE(tv_owf_lynx_256) {
  test_owf_tv(FAEST_LYNX_256S, owf_tvs::FAEST_LYNX_256S::key, owf_tvs::FAEST_LYNX_256S::input,
              owf_tvs::FAEST_LYNX_256S::output);
}

// 384-bit Lynx OWF test vectors are currently disabled in this branch.
