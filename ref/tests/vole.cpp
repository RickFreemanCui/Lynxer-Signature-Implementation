...
BOOST_DATA_TEST_CASE(vole_commit_verify, all_parameters, param_id) {
  BOOST_TEST_CONTEXT("Parameter set: " << faest_get_param_name(param_id)) {
    ...
    const unsigned int ell_hat       = params->l + (faest_is_lynx(params) ? 2 : 3) * params->lambda + UNIVERSAL_HASH_B_BITS;
    const unsigned int ell_hat_bytes = (ell_hat + 7) / 8;
    const auto com_size              = (faest_uses_short_leaf_commit(params) ? 2 : 3) * lambda_bytes;
