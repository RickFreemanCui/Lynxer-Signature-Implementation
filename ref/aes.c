...
static void shake_prg(const uint8_t* key, const uint8_t* iv, uint32_t tweak, uint8_t* out,
                      unsigned int seclvl, size_t outlen) {
  const size_t key_bytes = seclvl / 8;
  uint8_t internal_iv[IV_SIZE];
  memcpy(internal_iv, iv, IV_SIZE);
  add_to_upper_word(internal_iv, tweak);

  hash_context ctx;
  hash_init(&ctx, seclvl);
  hash_update(&ctx, key, key_bytes);
  hash_update(&ctx, internal_iv, sizeof(internal_iv));
  hash_final(&ctx);
...
void prg(const uint8_t* key, const uint8_t* iv, uint32_t tweak, uint8_t* out, unsigned int seclvl,
         size_t outlen) {
  if (seclvl == 384 || seclvl == 512) {
    shake_prg(key, iv, tweak, out, seclvl, outlen);
    return;
  }
...
void prg_2_lambda(const uint8_t* key, const uint8_t* iv, uint32_t tweak, uint8_t* out,
                  unsigned int seclvl) {
  if (seclvl == 384 || seclvl == 512) {
    shake_prg(key, iv, tweak, out, seclvl, seclvl * 2 / 8);
    return;
  }
...
void prg_4_lambda(const uint8_t* key, const uint8_t* iv, uint32_t tweak, uint8_t* out,
                  unsigned int seclvl) {
  if (seclvl == 384 || seclvl == 512) {
    shake_prg(key, iv, tweak, out, seclvl, seclvl * 4 / 8);
    return;
  }
