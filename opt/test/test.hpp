...
template <typename P> inline std::vector<unsigned char> benchmark_message_for_secpar()
{
    std::vector<unsigned char> message(2 * P::secpar_bytes);
    for (size_t i = 0; i < message.size(); ++i)
        message[i] = static_cast<unsigned char>((0x5a + 17 * i) & 0xff);
    return message;
}
